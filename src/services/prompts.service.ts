...
export class PromptsService extends BaseService {
	...
	async renderPrompt(
		...
		return this.post<RenderPromptResponse>(`/prompts/${promptId}/render`, data);
	}
	...
	validateBillingMetadata(
		...
		if (metadata.app && !validApps.includes(metadata.app)) {
			warnings.push(
				`Unrecognized app: "${metadata.app}". Expected one of: ${validApps.join(", ")}`,
			);
		}
		...
		if (metadata.env && !validEnvs.includes(metadata.env)) {
			warnings.push(
				`Unrecognized env: "${metadata.env}". Expected one of: ${validEnvs.join(", ")}`,
			);
		}
