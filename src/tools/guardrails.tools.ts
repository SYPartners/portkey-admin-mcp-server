	...
	server.tool(
		...
		"List all guardrails in your Portkey organization with optional filtering by workspace or organization",
		{
			workspace_id: z
				...
				.uuid()
				.optional()
				.describe("Filter guardrails by workspace UUID"),
			organisation_id: z
				.string()
				.uuid()
				.optional()
				.describe("Filter guardrails by organization UUID"),
			page_size: z
				.number()
				.min(1)
				.max(1000)
	...
	server.tool(
		...
		"Create a new guardrail with specified checks and actions for content moderation and security",
		{
			...
			workspace_id: z
				...
				.uuid()
				.optional()
				.describe("Workspace UUID to create the guardrail in"),
			organisation_id: z
				.string()
				.uuid()
				.optional()
				.describe("Organisation UUID (required if workspace_id not provided)"),
		},
		async (params) => {
			try {
