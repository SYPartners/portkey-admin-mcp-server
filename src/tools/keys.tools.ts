		...
		async (params) => {
			try {
				const result = await service.createVirtualKey({
					...
					usage_limits:
						params.credit_limit || params.alert_threshold
							? {
							...
							: undefined,
					rate_limits: params.rate_limit_rpm
						? [{ type: "requests", unit: "rpm", value: params.rate_limit_rpm }]
						: undefined,
				});
				...
				const result = await service.updateVirtualKey(params.slug, {
					...
					usage_limits:
						...
						params.alert_threshold !== undefined
							...
							: undefined,
					rate_limits:
						params.rate_limit_rpm !== undefined
							? [
									{
										type: "requests",
										unit: "rpm",
										value: params.rate_limit_rpm,
									},
								]
							: undefined,
				});
				...
				const result = await service.createApiKey(
					...
					params.sub_type,
					{
						...
						rate_limits: params.rate_limit_rpm
							...
							: undefined,
						defaults:
							params.default_config_id || params.default_metadata
								? {
