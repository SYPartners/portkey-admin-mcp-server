...
export function registerLimitsTools(
	...
	server.tool(
		...
		"Create a new usage limit to control resource consumption within a time period",
		{
			...
			metric: z
				.string()
				.describe(
					"The metric to limit (e.g., 'tokens', 'requests', 'cost_usd')",
				),
			period: z
				.string()
				.describe("The time period for the limit (e.g., 'daily', 'monthly')"),
		},
		async (params) => {
			try {
	...
	server.tool(
		...
		"Update an existing usage limit's configuration",
		{
			...
			metric: z.string().optional().describe("New metric to limit"),
			period: z.string().optional().describe("New time period for the limit"),
			status: z
	...
	server.tool(
		...
		"Create a new rate limit to control request frequency within a time window",
		{
			...
			metric: z
				.string()
				.describe("The metric to rate limit (e.g., 'requests', 'tokens')"),
			window: z
				.string()
				.describe(
					"The time window for the rate limit (e.g., '1m', '1h', '1d')",
				),
		},
		async (params) => {
			try {
	...
	server.tool(
		...
		"Update an existing rate limit's configuration",
		{
			...
			metric: z.string().optional().describe("New metric to rate limit"),
			window: z
				.string()
				.optional()
